...
regressorLR.fit(X, Y)
plt.scatter(horizontal, regressorLR.predict(X)[orden], color='blue', linewidth=3, label='LR')

regressorSVR = SVR(kernel='rbf',C=1e1,epsilon=1)
regressorSVR.fit(X, Y)
plt.scatter(horizontal, regressorSVR.predict(X)[orden], color='green', linewidth=3, label='SVR')

regressorRF = RandomForestRegressor()
regressorRF.fit(X, Y)
plt.scatter(horizontal, regressorRF.predict(X)[orden], color='red', linewidth=3, label='RF')

regressorMLP = MLPRegressor()
regressorMLP.fit(X, Y)
plt.scatter(horizontal, regressorMLP.predict(X)[orden], color='yellow', linewidth=3, label='MLP')
